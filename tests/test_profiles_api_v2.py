...
profile_id = r.json().get('id') if r.status_code in [200, 201] and 'id' in r.json() else None
print(f"   状态: {r.status_code}")
if profile_id:
    print(f"   档案ID: {profile_id}")
else:
    print(f"   响应: {r.text[:200] if len(r.text) > 0 else 'Empty'}")
...
if profile_id:
    ...
    print(f"   状态: {r.status_code}")
    contact_id = r.json().get('id') if r.status_code in [200, 201] and 'id' in r.json() else None
