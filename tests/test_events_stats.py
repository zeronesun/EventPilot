...
def test_events_stats_and_complete():
    ...
    # 3. 测试统计端点
    print("\n3. 测试活动统计端点:")
    r = requests.get(f"{BASE_URL}/events/{event_id}/statistics/", headers=headers)
    ...
    # 4. 测试风险评估端点
    print("\n4. 测试风险评估端点:")
    r = requests.get(f"{BASE_URL}/events/{event_id}/risk/", headers=headers)
    print(f"   状态码: {r.status_code}")
    if r.status_code == 200:
        risk = r.json()
    ...
    if r.status_code == 200:
        ...
        if candidate_event:
            ...
            # 测试完成端点
            r = requests.post(f"{BASE_URL}/events/{test_event_id}/complete/", headers=headers)
            print(f"\n6. 测试活动完成端点:")
            print(f"   状态码: {r.status_code}")
            if r.status_code == 200:
                result = r.json()
