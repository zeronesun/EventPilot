...
详细的开发指南、部署文档、API 参考和测试文档请访问：[EventPilot 文档中心](docs/INDEX.md)

### 技术栈

#### 后端
- **框架**：Django 5 + REST Framework
- **数据库**：PostgreSQL 15
- **缓存**：Redis
- **认证**：JWT + Token认证
...
- **基于角色的访问控制（RBAC）**：5级角色
  ...
  - ResourceMember（资源执行）
  - Observer（观察者/财务）
- **15+ 细粒度权限**：如 `view_events`, `change_tasks`, `delete_checklists` 等
- **资源级权限控制**，支持 `has_perm` 动态检查

### 安全防护
- **WebSocket消息签名验证**，防止中间人攻击
- **XSS/CSRF** 全面防护
- **速率限制** 和DoS防护
- **完整审计日志** 记录所有敏感操作
...
- **v1.0 MVP** (已完成)：基础活动、任务、核验清单管理
- **v1.1** (已完成)：关联方档案、智能方案生成、结构化复盘、知识库
- **v1.2** (当前开发)：知识推荐、数据仪表盘、微信深度集成
- **v2.0+** (规划)：AI深度集成、自动化流程引擎
